  ...
  ngOnInit()
  {
    ...
    this.shared.getEmittedValue().subscribe(value=>
    {
        ...
        if(this.route_changes.page!='Logout')
        {
          ...
          {
            if(this.route_changes.page == 'Dashboard' ){
              this.items.splice(1,this.items.length);
              this.items.push({label:"Story Board",url:"/project/"+this.route_changes.params+"/list"});
            }else{
              for (var key in this.items[0]) 
              {
                this.items[0].url="/project/"+this.route_changes.params;
                this.items[0].label=this.route_changes.params;
              }  
            }
          }
          if(this.route_changes.type=="New" && this.items.length>0 && this.route_changes.page == 'Dashboard')
          {
          ...
          if(this.route_changes.projectName!=''  && this.status!=true && this.project!='') //newly changed
          {
             ...
             { 
               this.items.push({label:this.route_changes.projectName,url:"/project/"+this.route_changes.projectName,queryString:''});
           ...
           { 
             ...
             if( this.items[0].type != "Other"){
               if(this.route_changes.type=="Story" ){
                 this.items.splice(1,this.items.length);    
                 this.items.push({label:"Story Board",url:"/project/"+this.route_changes.projectName+"/list",queryString:''});
           ...
           {
             if( (this.items[key].label=='#'+this.route_changes.params || this.route_changes.params==''))
             {
